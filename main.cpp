...
int main()
{
    ...
    char prev=' ';//Предыдущий рассматриваемый символ при считывании из файла
    ...
    while(1)//Основной цикл работы программы
    {
        while(1)//Цикл для проверки правильности введенной команды в меню
        {
            ...
            if(choise>6||choise<1)//Такой команды в меню нет, ошибка
            {
               cout << "Incorrect! Press any button...\n";
               _getch();
               system("cls");
        ...
        system("cls");
            break;
        case 5: system("cls");//Считывание 3 предложений из файла, их вывод в обратном порядке
        sizes[0]=0;//
        sizes[1]=0;//Обнуление размеров предложений
        sizes[2]=0;//
        prev=' ';//Установка пока что не существующего пред. символа
        ...
        {
           if((ch!='\n'&&ch!='\0'&&ch!='.'&&ch!='!'&&ch!='?')&&(prev=='\n'||prev=='\0'||prev=='.'||prev=='!'||prev=='?'))//Текущий символ - не разделитель, чего нельзя сказать о предыдущем
           {
              ...
              prev=ch;//Запоминание пред. символа
